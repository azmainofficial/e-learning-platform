...
export default function Show({ auth, project, students }) {
    const [isTaskModalOpen, setIsTaskModalOpen] = useState(false);
    const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);
    const [selectedTask, setSelectedTask] = useState(null);
    const fileInputRef = useRef(null);

    const { data: taskData, setData: setTaskData, post: postTask, patch, put, delete: destroy, reset: resetTask, processing: taskProcessing } = useForm({
        title: '',
        description: '',
        status: 'todo',
        priority: 'medium',
    ...
    const submitTask = (e) => {
        ...
        if (selectedTask) {
            ...
            postTask(route('tasks.store', project.id), {
    ...
    return (
        ...
        >
                ...
                <div className="max-w-[1600px] w-full mx-auto sm:px-6 lg:px-8 h-full flex flex-col">
                    ...
                    <div className="flex justify-between items-center mb-6 pl-2">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1 font-medium">Manage project tasks</p>
                        </div>
                        <div className="flex -space-x-3">
                            {project.users.map(user => (
                                <div key={user.id} className="w-10 h-10 rounded-full bg-gray-400 border-2 border-white flex items-center justify-center text-gray-900 text-xs font-bold shadow-sm" title={user.name}>
                        ...
                        <div className="flex gap-6 min-w-max h-full items-start">
                            {STATUS_COLUMNS.map((col) => (
                                <div key={col.id} className="w-[320px] flex flex-col bg-gray-50/50 rounded-xl border border-gray-200 h-full max-h-full">
                                    <div className={`px-4 py-3 border-b ${col.border} rounded-t-xl flex justify-between items-center`}>
                                        ...
                                        <button onClick={() => openCreateTask(col.id)} className="p-1 hover:bg-gray-200 rounded-md transition-colors text-gray-500">
                                    ...
                                    <div className="flex-1 overflow-y-auto p-3 space-y-3">
                                        {project.tasks.filter(t => t.status === col.id).map((task) => (
                                            <div
                                                ...
                                                className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm hover:shadow-md cursor-pointer transition-shadow"
                                            >
            ...
            {/* Task Management Modal (Create/Edit) */}
            {isTaskModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-gray-900/60 " onClick={() => setIsTaskModalOpen(false)} />
                    <div className="relative bg-white rounded-xl w-full max-w-xl p-8 shadow-md">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-bold text-gray-900">{selectedTask ? 'Edit Task' : 'Create Task'}</h2>
                            <button onClick={() => setIsTaskModalOpen(false)} className="text-gray-500 hover:text-gray-600">
                        ...
                        <form onSubmit={submitTask} className="space-y-5">
                            <div>
                                ...
                                <input type="text" value={taskData.title} onChange={e => setTaskData('title', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                                <textarea value={taskData.description} onChange={e => setTaskData('description', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm min-h-[100px]" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Priority</label>
                                    <select value={taskData.priority} onChange={e => setTaskData('priority', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                ...
                                <div>
                                    ...
                                    <select value={taskData.assigned_to} onChange={e => setTaskData('assigned_to', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" value={taskData.due_at} onChange={e => setTaskData('due_at', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Status</label>
                                    <select value={taskData.status} onChange={e => setTaskData('status', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                        {STATUS_COLUMNS.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div className="pt-4 flex justify-end gap-3">
                                <button type="button" onClick={() => setIsTaskModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">Cancel</button>
                                <button type="submit" disabled={taskProcessing} className="px-4 py-2 text-sm font-medium text-gray-900 bg-blue-600 rounded-md hover:bg-blue-700 shadow-sm disabled:opacity-50">
            ...
            {isDetailModalOpen && currentTask && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-gray-900/60 " onClick={() => setIsDetailModalOpen(false)} />
                    ...
                    <div className="relative bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden shadow-md">
                        ...
                        <div className="px-8 py-6 border-b border-gray-200 flex justify-between items-start bg-gray-50/50">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => { setIsDetailModalOpen(false); openEditTask(currentTask); }} className="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-blue-600 hover:border-blue-300 shadow-sm transition-all">
                                    <PencilSquareIcon className="w-5 h-5" />
                                </button>
                                <button onClick={() => deleteTask(currentTask.id)} className="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-red-600 hover:border-red-300 shadow-sm transition-all">
                                    <TrashIcon className="w-5 h-5" />
                                </button>
                                <button onClick={() => setIsDetailModalOpen(false)} className="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-50 shadow-sm transition-all ml-4">
